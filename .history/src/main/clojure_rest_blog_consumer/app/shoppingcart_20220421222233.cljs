...
(defn remove-item [item]
  (let [new-cart (remove #(= (:id item) (:id %)) (:cart @state/state))]
    (println (count new-cart))
  ...
  (let [item (:item cart-item)]
    ...
    [:div {:class "flex justify-between items-center mt-6 pt-6 border-t"}
     ...
     [:div {:class "flex justify-center items-center"}
      ...
      [:button {:class "w-10 px-2 py-0 text-black-100 rounded" :on-click #(remove-item item)} "X"]
      [:div " "
        ...
        [:div {:class "col-span-2 p-5"}
         ...
         [:div {:class "flex justify-between items-center mt-6 pt-6 border-t"}
          [:div {:class "flex justify-center items-center"}
           ...
           [:span {:class "text-sm title-font text-gray-400 mr-1"} "Totalt:"]
           " "]
          [:div {:class "flex justify-center items-center"}
