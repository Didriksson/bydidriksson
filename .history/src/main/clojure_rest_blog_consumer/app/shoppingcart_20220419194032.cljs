      ...
      [:div {:class "w-full p-4 px-5 py-5"}
       [:div {:class "md:grid md:grid-cols-3 gap-2 "}
        ...
        [:div {:class " p-5 bg-gray-800 rounded overflow-visible"}
         [:span {:class "text-xl font-medium text-gray-100 block pb-3"} "Card Details"]
         [:span {:class "text-xs text-gray-400 "} "Card Type"]
         [:div {:class "overflow-visible flex justify-between items-center mt-2"}
          [:div {:class "rounded w-52 h-28 bg-gray-500 py-2 px-4 relative right-10"}
           [:span {:class "italic text-lg font-medium text-gray-200 underline"} "VISA"]
           ...
           [:div {:class "flex justify-between items-center mt-3"}
            [:span {:class "text-xs text-gray-200"} "Giga Tamarashvili"]
            [:span {:class "text-xs text-gray-200"} "12/18"] " "]]
          [:div {:class "flex justify-center items-center flex-col"}
           [:img {:src "https://img.icons8.com/color/96/000000/mastercard-logo.png", :width "40", :class "relative right-5"}]
           [:span {:class "text-xs font-medium text-gray-200 bottom-2 relative right-5"} "mastercard."] " "]]
         [:div {:class "flex justify-center flex-col pt-3"}
          [:label {:class "text-xs text-gray-400 "} "Name on Card"]
         ...
         [:div {:class "flex justify-center flex-col pt-3"}
          [:label {:class "text-xs text-gray-400 "} "Card Number"]
         ...
         [:div {:class "grid grid-cols-3 gap-2 pt-2 mb-3"}
          [:div {:class "col-span-2 "}
           [:label {:class "text-xs text-gray-400"} "Expiration Date"]
          ...
          [:div
           [:label {:class "text-xs text-gray-400"} "CVV"]
         ...
         [:button {:class "h-12 w-full bg-blue-500 rounded focus:outline-none text-white hover:bg-blue-600"} "Check Out"]]]]]]]])