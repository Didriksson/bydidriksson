...
(defn render-message [message]
  [:p (str "Id: " (get message :id) " " (get message :author) ": " (get message :message))]
)
  ...
  [:div {:class "-my-8 divide-y-2 divide-gray-100"}
   [:div {:class "py-8 flex flex-wrap md:flex-nowrap"}
    [:div {:class "md:w-64 md:mb-0 mb-6 flex-shrink-0 flex flex-col"}
     ...
     [:span {:class "mt-1 text-gray-500 text-sm"} "12 Jun 2019"]]
   ...
   [:div {:class "py-8 flex flex-wrap md:flex-nowrap"}
    [:div {:class "md:w-64 md:mb-0 mb-6 flex-shrink-0 flex flex-col"}
     [:span {:class "font-semibold title-font text-gray-700"} "CATEGORY"]
     [:span {:class "text-sm text-gray-500"} "12 Jun 2019"]]
    [:div {:class "md:flex-grow"}
     [:h2 {:class "text-2xl font-medium text-gray-900 title-font mb-2"}
