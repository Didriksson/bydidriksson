...
(defonce emailform (atom { :email "" :name "" :message ""}))
(defonce sent (atom false))
   ...
   [:div {:class "container px-5 py-24 mx-auto"}
    ...
    [:div {:class "lg:w-1/2 md:w-2/3 mx-auto"}
     [:div {:class "flex flex-wrap -m-2"}
      ...
      [:div {:class "p-2 w-full"}
       (if @sent
         [:button {:class "flex mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg" :disabled true} "Tack!"]
         [:button {:class "flex mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg disabled:opacity-50"
                   :on-click #(skicka)} "Skicka"])]
