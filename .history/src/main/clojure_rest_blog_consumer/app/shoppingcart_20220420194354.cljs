        ...
        [:div {:class "col-span-2 p-5"}
         ...
         [:div {:class "flex justify-between items-center mt-6 pt-6 border-t"}
          ...
          [:div {:class "flex justify-center items-center"}
           ...
           [:span {:class "pr-8 text-lg font-bold text-gray-800"} (.toFixed (reduce + (map js/parseFloat (map :price (:cart @state/state)))) 2)]
           [:div {:class "w-10"} " "]
           " "]          
          ]
         [:div {:class "flex justify-between items-center mt-6 pt-6 border-t"}
          [:button {:class "flex ml-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded" :on-click #(swap! state/state update :cart conj @item)} "Lägg till i varukorg"]]]]]]]]])